...
## Current State: Two Browser Tool Abstractions

`cube-standard` currently has **two separate browser tool abstractions** that coexist and serve different roles. Neither is the target design above — they're legacy.

### `tool.py:BrowserTool` (line 384)

An abstract base class that inherits from `Tool` (Pydantic model):

```python
class BrowserTool(Tool):
    """Abstract base for browser tools used by web-based tasks."""
...
- `cube-harness/src/cube_harness/tools/browsergym.py`

### `tools/browser.py:AbstractBrowserTool`

A `Protocol` (structural subtyping, `@runtime_checkable`). Richer interface:
...
**Used by task code:**
- `cube-harness/cubes/miniwob/src/miniwob_cube/task.py`

### Comparison

| | `tool.py:BrowserTool` | `tools/browser.py:AbstractBrowserTool` |
|---|---|---|
| Style | ABC inheriting `Tool` (Pydantic) | `Protocol` (structural subtyping) |
| Interface | `session`, `noop`, `page_obs` | `reset`, `close`, `execute_action`, `goto`, `evaluate_js`, `page_obs` |
| Used by | Concrete impl classes | Task-level code |
| Lives in | `tool.py` (generic infrastructure) | `tools/browser.py` (domain-specific) |

### Why this is inconsistent

`BrowserTool` is a domain-specific class sitting in `tool.py` alongside generic infrastructure (`Tool`, `AsyncTool`, `ToolConfig`). It belongs in `tools/browser.py`.

Moving it would require updating imports in `cube-browser-tool` and `cube-harness`. It is also a different abstraction style (ABC vs Protocol) — the target design above replaces both with `BgymTool` / `BrowserToolConfig` and drops the Protocol approach in favour of concrete classes with a `BrowserSession`.
