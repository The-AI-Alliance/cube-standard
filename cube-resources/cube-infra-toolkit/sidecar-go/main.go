...
//
// Same HTTP protocol as _exec_relay_server.py; drop-in replacement that needs
...
//
// Security posture matches the Python relay — see _exec_relay_server.py header.
package main
...
func loadToken() {
	path := os.Getenv("CUBE_EXEC_RELAY_TOKEN_FILE")
	if path == "" {
		fmt.Fprintln(os.Stderr, "CUBE_EXEC_RELAY_TOKEN_FILE not set")
		os.Exit(2)
	}
...
func handleExec(w http.ResponseWriter, r *http.Request) {
	...
	for _, e := range env {
		if !strings.HasPrefix(e, "CUBE_EXEC_RELAY_TOKEN_FILE=") {
...
func main() {
	...
	port := os.Getenv("CUBE_EXEC_RELAY_PORT")
	if port == "" {
		port = defaultPort
	}
