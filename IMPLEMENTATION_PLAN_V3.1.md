...
- Decision on FastAPI vs MCP Server for task servers
- TaskSession integration
...
from cube.types import SpawnRequest, TaskRequest
...
bench.setup(
    ...
    server_mode=False  # Python API mode (no HTTP server)
)

# Call benchmark-level methods directly via Python
metadata = bench.info()  # Returns BenchmarkMetadata
print(f"Benchmark: {metadata.name} v{metadata.version}")

# List tasks
task_list = bench.list_tasks(TaskRequest())  # Returns TaskListResponse
print(f"Found {task_list.total} tasks")

# Spawn task (requires custom implementation in server_mode=False)
# Phase 1: benchmark.spawn() raises NotImplementedError in Python mode
# Benchmark authors must implement custom spawn logic for Python mode
# OR use server_mode=True to get automatic spawn via SessionManager

# Phase 2: Will also support task-level Python API via TaskSession
```

**What works in Phase 1:**
- ✅ Benchmark-level Python API: info(), list_tasks()
- ✅ HTTP mode: spawn(), status(), shutdown() (via SessionManager)
- ❌ Python mode spawn() (benchmark authors must implement custom logic)
- ❌ Task-level Python API (Phase 2): via TaskSession directly
