...
### What's Missing ❌
1. **Wrapper types for TaskSession methods** (in types.py):
   - `ToolListResponse`, `ToolCallRequest`, `ToolCallResponse`
   - `ResourceListResponse`, `ResourceReadResponse`
2. **Server infrastructure**: No FastAPI servers exist yet
3. **Session management**: No tracking of spawned task servers
4. **Benchmark spawn/shutdown implementations**: Currently abstract methods

## Implementation Plan

### Phase 1: Add Missing Type Definitions

**File**: [src/cube/types.py](src/cube/types.py)

Add missing wrapper types that TaskSession methods already use (currently causing import errors):

```python
# After line 379 (after MCP imports), add:

# =============================================================================
# CUBE Wrapper Types for Task-Level API
# =============================================================================

class ToolListResponse(BaseModel):
    """
    Response schema for tools/list endpoint.
    Wraps MCP Tool list for CUBE API.
    """
...
class ToolCallRequest(BaseModel):
    """
    Request schema for tools/call endpoint.
    Matches MCP CallToolRequestParams format.
    """
...
class ToolCallResponse(BaseModel):
    """
    Response schema for tools/call endpoint.
    Wraps MCP CallToolResult for CUBE API.
    """
    content: list[MCPTextContent] = Field(..., description="Tool execution result content")
    isError: bool = Field(default=False, description="Whether execution resulted in error")


class ResourceListResponse(BaseModel):
    """
    Response schema for resources/list endpoint.
    Wraps MCP Resource list for CUBE API.
    """
...
class ResourceReadResponse(BaseModel):
    """
    Response schema for resources/read endpoint.
    Wraps MCP resource contents for CUBE API.
    """
    contents: list[MCPTextResource | MCPBlobResource] = Field(
        ..., description="Resource contents (text or binary)"
    )
```

**Why**: These types are already imported and used by TaskSession but don't exist in types.py, causing import errors.

### Phase 2: Create Server Infrastructure
...
### Phase 3: Integrate Server with Benchmark Class
...
result = session.call_tool(ToolCallRequest(name="click", arguments={"x": 100}))
...
1. **[src/cube/types.py](src/cube/types.py)** - Add missing wrapper types (ToolListResponse, ToolCallRequest, etc.)
2. **[src/cube/server/session_manager.py](src/cube/server/session_manager.py)** - Create session management (NEW)
3. **[src/cube/server/task_server.py](src/cube/server/task_server.py)** - Create task FastAPI server (NEW)
4. **[src/cube/server/benchmark_server.py](src/cube/server/benchmark_server.py)** - Create benchmark FastAPI server (NEW)
