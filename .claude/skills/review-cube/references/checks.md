...
- S-50: Cube ships a `tool` property override or `isinstance(self.tool, FooTool)` asserts to narrow `self.tool` to a specific surface. The cube can drop this boilerplate by parameterizing the base class — `class FooTask(Task[FooMeta, FooTool])` — which types `self.tool` correctly without an override. See [task/spec.md `Task` signature](../../../../openspec/specs/task/spec.md).
