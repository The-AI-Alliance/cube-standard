...
class AsyncBrowserActionSpace(ABC):
    """Abstract base for async browser tools that locate elements via CSS selectors.

    Mirrors ``BrowserActionSpace`` but all action methods are coroutines, matching
    the async Playwright API. Subclasses (e.g. ``AsyncPlaywrightTool``) must implement
    every method. The ``@tool_action`` decoration on each abstract method is inherited
    by overrides, so concrete classes do not need to repeat the decorator.
    """
